...
function searchMovies(
  ...
  if (filteredMovies.length === 0) {
    pageTextSetter('Ничего не найдено')
    currentStateSetter('')
  } else {
    // иначе:
    ...
    filteredMoviesSetter(filteredMovies)
    if (displayedMoviesSetter) {
      displayedMoviesSetter(filteredMovies.slice(0, calcInitialMovies()))
    }
    currentStateSetter('loaded')
  }
}
